...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  void _handleRestore() async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(success ? 'تمت استعادة النسخة بنجاح. يرجى إعادة تشغيل التطبيق بالكامل.' : 'فشل استعادة النسخة الاحتياطية'),
        backgroundColor: success ? Colors.green : Colors.red,
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: const Row(
          children: [
            ...
            SizedBox(width: 12),
            Text('إلغاء التفعيل (خطر!)', style: TextStyle(color: Colors.red, fontWeight: FontWeight.bold)),
          ],
        ),
        content: const Text(
        ...
        actions: [
          ...
          ElevatedButton.icon(
            ...
            label: const Text('تأكيد الإلغاء والإغلاق', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
            onPressed: () async {
              final db = sl<AppDatabase>();
              await (db.update(db.license)..where((t) => t.id.equals(1))).write(
                const LicenseCompanion(isActivated: drift.Value(false), licenseKey: drift.Value('')),
              );
              exit(0);
            },
          ),
      ...
      body: BlocConsumer<SettingsBloc, SettingsState>(
        listener: (context, state) {
          if (state is SettingsSavedSuccess) {
            ...
            ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('خطأ: ${state.message}'), backgroundColor: Colors.red));
          }
        },
                  ...
                  child: ListView(
                    ...
                    children: [
                      ...
                      _SettingsSectionCard(
                        ...
                        icon: Icons.store,
                        children: [
                          _CustomSettingsField(controller: _restaurantNameController, label: 'اسم المطعم (يظهر في الفاتورة)', hint: 'مثال: مطعم الشيف', icon: Icons.restaurant),
                          const SizedBox(height: 16),
                          _CustomSettingsField(controller: _taxNumberController, label: 'الرقم الضريبي', hint: 'مثال: 123-456-789', icon: Icons.numbers),
                        ],
                      ),
                      ...
                      _SettingsSectionCard(
                        ...
                        children: [
                          Row(
                            children: [
                              Expanded(child: _CustomSettingsField(controller: _taxController, label: 'نسبة الضريبة المضافة (%)', hint: '14', icon: Icons.percent, isNumber: true)),
                      ...
                      ElevatedButton.icon(
                        ...
                        label: const Text('حفظ كافة الإعدادات', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
                            ...
                            children: [
                              Expanded(
                                child: ElevatedButton.icon(
                                  ...
                                  label: const Text('إنشاء نسخة احتياطية', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                              ...
                              Expanded(
                                child: ElevatedButton.icon(
                                  style: ElevatedButton.styleFrom(backgroundColor: Colors.orange.shade800, foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(vertical: 20), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
                                  icon: const Icon(Icons.cloud_upload),
                                  label: const Text('استعادة نسخة سابقة', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                        ...
                        children: [
                          SizedBox(
                            ...
                            child: OutlinedButton.icon(
                              style: OutlinedButton.styleFrom(foregroundColor: Colors.red, side: const BorderSide(color: Colors.red, width: 2), padding: const EdgeInsets.symmetric(vertical: 20), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
                              icon: const Icon(Icons.phonelink_erase),
                              label: const Text('إلغاء تفعيل النسخة الحالية (للنقل لجهاز آخر)', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
    ...
    return Row(
      children: [
        ...
        Container(
          height: 60,
          width: 60,
          decoration: BoxDecoration(color: Colors.teal, borderRadius: BorderRadius.circular(12), boxShadow: [BoxShadow(color: Colors.teal.withOpacity(0.3), blurRadius: 8, offset: const Offset(0, 4))]),
          child: IconButton(
            icon: _isLoadingPrinters ? const SizedBox(width: 24, height: 24, child: CircularProgressIndicator(color: Colors.white, strokeWidth: 2)) : const Icon(Icons.refresh, color: Colors.white, size: 28),
...
class _CustomSettingsField extends StatelessWidget {
  ...
  const _CustomSettingsField({required this.controller, required this.label, required this.hint, required this.icon, this.isNumber = false});
...
class _SettingsSectionCard extends StatelessWidget {
  ...
  const _SettingsSectionCard({required this.title, required this.icon, required this.children, this.titleColor});
          ...
          Row(
            children: [
              Container(
                ...
                decoration: BoxDecoration(color: (titleColor ?? Colors.teal).withOpacity(0.1), shape: BoxShape.circle),
                child: Icon(icon, color: titleColor ?? Colors.teal, size: 28),
              ),
              const SizedBox(width: 12),
              Text(title, style: TextStyle(fontSize: 20, fontWeight: FontWeight.w900, color: titleColor ?? Colors.black87)),
            ],
          ),
          const Padding(padding: EdgeInsets.symmetric(vertical: 16.0), child: Divider(height: 1, thickness: 1)),
          ...children,
        ],
      ),
    );
    ...
    return Center(
      child: ConstrainedBox(
        ...
        child: ListView(
          padding: const EdgeInsets.all(24.0),
          children: [
            CustomShimmer.rectangular(height: 250, shapeBorder: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16))),
            const SizedBox(height: 24),
            CustomShimmer.rectangular(height: 300, shapeBorder: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16))),
            const SizedBox(height: 24),
            CustomShimmer.rectangular(height: 200, shapeBorder: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16))),
          ],
        ),
      ),
    );
